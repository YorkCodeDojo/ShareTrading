...
{
    public class WhenGettingThePriceForProductA : IClassFixture<WebApplicationFactory<Startup>>
    {
        readonly WebApplicationFactory<Startup> _factory;

        public WhenGettingThePriceForProductA(WebApplicationFactory<Startup> factory)
        {
            _factory = factory;
        }

        [Fact]
        public async Task The_Price_Is_Fixed_Regardless_Of_The_Time()
        {
            ...
            var outputFile = @"c:\temp\ProductA.csv";
            ...
            while (currentTime <= endTime)
            {
                ...
                var response = await client.GetAsync($"/api/Products/ProductA");
