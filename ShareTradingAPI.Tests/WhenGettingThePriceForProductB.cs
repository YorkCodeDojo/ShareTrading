...
{
    public class WhenGettingThePriceForProductB : IClassFixture<WebApplicationFactory<Startup>>
    {
        readonly WebApplicationFactory<Startup> _factory;

        public WhenGettingThePriceForProductB(WebApplicationFactory<Startup> factory)
        {
            _factory = factory;
        }
        ...
        public async Task The_Price_Is_Varies_In_A_Known_Way()
        {
            ...
            var outputFile = @"c:\temp\ProductB.csv";
            ...
            while (currentTime <= endTime)
            {
                ...
                var response = await client.GetAsync($"/api/Products/ProductB");
